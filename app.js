...
function matchesFilters(brewery) {
  const text = [brewery.name, brewery.category, brewery.address, brewery.area, brewery.summary, brewery.details, brewery.bestFor, brewery.tip].join(" ").toLowerCase();
      ...
      <article class="card ${checked ? "done" : ""}" data-id="${escapeHtml(b.id)}">
        <div class="cardTop">
          ...
          <div>
            ...
            <div class="badges">${badge(b.category)}${badge(b.area, "green")}${badge("Address included", "gold")}</div>
          </div>
        </div>
...
async function copyCheckedList() {
  ...
  const text = checked.map(b => `${b.rank}. ${b.name} — ${b.address}`).join("\n");
